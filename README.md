...
X / Twitter 现在走 `baoyu-url-to-markdown`。默认情况下，它会尝试自行拉起临时浏览器抓取页面；如果你需要复用自己当前已登录的 Chrome 会话，再手动开启 9222 调试端口。若提取仍然失败，常见原因是页面需要登录、当前会话权限不足，或页面本身返回了不完整内容。你也可以直接把内容复制粘贴给 agent 处理。
